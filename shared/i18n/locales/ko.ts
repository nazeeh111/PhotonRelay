...
export const messages: Messages = {
  ...
  home: {
    title: "PhotonRelay — 빛으로 파일 전송",
    metaDescription:
      "화면과 카메라만으로 두 기기 사이에 파일이나 텍스트를 전송합니다. 파운틴 코딩된 애니메이션 QR 코드로 전해지며, 중간에 네트워크 경로가 없습니다.",
    ogDescription: "화면과 카메라만으로 두 기기 사이에 파일이나 텍스트를 전송합니다. 파운틴 코딩된 애니메이션 QR 코드로 전해지며, 중간에 네트워크 경로가 없습니다.",
    ...
    shareSite: "PhotonRelay 공유",
    supportTitle: "무료, 오픈 소스, 광고 없음",
    supportBodyHtml:
      'PhotonRelay이 도움이 되었다면 <a href="https://buymeacoffee.com/bashalarmist" target="_blank" rel="noopener noreferrer">커피 한 잔</a>으로 응원해 주세요.',
  ...
  send: {
    ...
    shareTitleData: "PhotonRelay — 수신기",
  ...
  verdicts: {
    olderSender: (version) =>
      `상대 화면이 이전 버전의 PhotonRelay 형식(v${version})을 보내고 있습니다. 보내는 기기를 업데이트하세요.`,
    newerSender: (version) =>
      `상대 화면이 더 새로운 PhotonRelay 형식(v${version})을 보내고 있습니다. 받으려면 이 앱을 업데이트하세요.`,
    unsupportedFlags:
      "이 스트림은 이 버전에서 읽을 수 없는 PhotonRelay 기능을 사용합니다. 받으려면 이 앱을 업데이트하세요.",
  },
  ...
  i18n: {
    ...
    switchOffer: "PhotonRelay을 한국어로 볼 수 있습니다.",
