...
export const messages: Messages = {
  ...
  home: {
    title: "PhotonRelay — передача файлов светом",
    metaDescription:
    ...
    ogDescription: "Отправляйте файлы и текст между двумя устройствами, используя только экран и камеру. Анимированные QR-коды с фонтанным кодированием, без сетевого соединения между устройствами.",
    ...
    shareSite: "Поделиться PhotonRelay",
    supportTitle: "Бесплатно, открытый код, без рекламы",
    supportBodyHtml:
      'Если PhotonRelay вам пригодился, можете <a href="https://buymeacoffee.com/bashalarmist" target="_blank" rel="noopener noreferrer">угостить меня кофе</a>.',
  ...
  send: {
    ...
    shareTitleData: "PhotonRelay — приёмник",
  ...
  receive: {
    ...
    clearCache: "Очистить кэш PhotonRelay",
  ...
  verdicts: {
    olderSender: (version) =>
      `Передающий экран использует более старый формат PhotonRelay (v${version}). Обновите отправляющее устройство.`,
    newerSender: (version) =>
      `Передающий экран использует более новый формат PhotonRelay (v${version}). Обновите это приложение, чтобы принять поток.`,
    unsupportedFlags:
      "Этот поток использует возможность PhotonRelay, которую эта версия не умеет читать. Обновите это приложение, чтобы принять поток.",
  },
  ...
  i18n: {
    ...
    switchOffer: "PhotonRelay доступен на русском.",
