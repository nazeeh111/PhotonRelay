...
export const messages: Messages = {
  ...
  home: {
    title: "PhotonRelay — 光でファイルを転送",
    metaDescription:
      "画面とカメラだけで、2台のデバイス間でファイルやテキストを送れます。ファウンテン符号化されたアニメーションQRコードを使い、間にネットワーク経路はありません。",
    ogDescription: "画面とカメラだけで、2台のデバイス間でファイルやテキストを送れます。ファウンテン符号化されたアニメーションQRコードを使い、間にネットワーク経路はありません。",
    ...
    shareSite: "PhotonRelay を共有",
    supportTitle: "無料・オープンソース・広告なし",
    supportBodyHtml:
      'PhotonRelay が役に立ったら、<a href="https://buymeacoffee.com/bashalarmist" target="_blank" rel="noopener noreferrer">コーヒーを1杯おごる</a>ことで応援できます。',
  ...
  send: {
    ...
    shareTitleData: "PhotonRelay — 受信ページ",
  ...
  receive: {
    ...
    clearCache: "PhotonRelay のキャッシュを消去",
  ...
  verdicts: {
    olderSender: (version) =>
      `相手の画面は古い PhotonRelay 形式 (v${version}) で送信しています。送信側のデバイスを更新してください。`,
    newerSender: (version) =>
      `相手の画面は新しい PhotonRelay 形式 (v${version}) で送信しています。受信するには、このアプリを更新してください。`,
    unsupportedFlags:
      "このストリームは、このバージョンでは読み取れない PhotonRelay の機能を使用しています。受信するには、このアプリを更新してください。",
  },
  ...
  i18n: {
    ...
    switchOffer: "PhotonRelay は日本語でもご利用いただけます。",
    switchAction: "日本語で表示",
  },
};
